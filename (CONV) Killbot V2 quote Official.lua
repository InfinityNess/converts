...
getsynasset = getsynasset or getcustomasset or function() warn("no getcustomasset/getsynasset") end
...
warn([[
	Regain Control
		...
		And fight for (The city that we made)
		To take back (The power that you crave)
		
		
		A prequel to Killbot
		
		Created by 	Chron_a 						(UserId: 5719877) 			[ROBLOX]
			        Drahazar#6969 					(UserId: 138946947842179072)[DISCORD]
		Converted by table.insert                   (Tag: steam#6987 / table.insert#6987)[DISCORD]
		Reanimate by Gelatek                        (Tag: gelatek#9889)[Discord]
					
	Anyone else claiming to have created/converted this is lying to you.
	OUTDATED: Hats required are: 20011897, 3443038622, 3662265036 and 4645404679 (space cop, peru, indonesia and thailand international fedoras(or the dummy head from blazyyz ugc, 11263254795))
]])
-- -gh 20011897,3443038622,3662265036,4645404679 [-gh 20011897,11263254795  is a alternative] 
wait(1/60)
game:GetObjects("rbxassetid://"..4781464455)
--require(4781464455)()
...
local Char = game.Workspace:FindFirstChild("GelatekReanimate")
local char = Char
...
EffectFolder.Name='MainEffects'
EffectFolder.Parent=char
local FXFolder = Game:GetObjects("rbxassetid://"..11978486145)[1]
FXFolder.Parent=char
--local Vector3Sequence,Vector3SequenceKeypoint = Game:GetObjects("rbxassetid://"..11978453754)--unpack(Game:GetObjects("rbxassetid://"..11978453754))
function Vector3SequenceKeypointNew(time,value,envelope)
...
--local CFrameSequence,CFrameSequenceKeypoint = Game:GetObjects("rbxassetid://"..11978399021)--unpack(Game:GetObjects("rbxassetid://"..11978399021))
function CFrameSequenceKeypointNew(time,value,envelope)
...
local ExplosionDeb = Game:GetObjects("rbxassetid://"..11978514900)[1]
ExplosionDeb.Parent=char
local GibParticles = Game:GetObjects("rbxassetid://"..11978511342)[1]
GibParticles.Parent=char
local Movement = 8
local Sine = 0;
local Change = 1;
local wsVal = 8
local crushedHead = game:GetObjects("rbxassetid://"..11981519801)[1]
crushedHead.Parent=char
local BloodParticles = IN("Attachment")
local mist = game:GetObjects("rbxassetid://"..11981535144)[1]
mist.Parent = BloodParticles
for i,v in pairs(FXFolder:GetChildren()) do
    v.CFrame=CFrame.new(v.Position.X,v.Position.Y*50000,v.Position.Z)
end
for i,v in pairs(crushedHead:GetChildren()) do
    v.CFrame=CFrame.new(v.Position.X,v.Position.Y*50000,v.Position.Z)
end
ExplosionDeb.CFrame=CFrame.new(ExplosionDeb.Position.X,ExplosionDeb.Position.Y*50000,ExplosionDeb.Position.Z)
--// INSTANCE CREATORS \\--
...
local Gren = Game:GetObjects("rbxassetid://"..11978525394)[1]
...
local MusicData = {
    Parent=char,
    ID=getsynasset("Table_Insert_Converts/scripts/music/KillbotV2.mp3");--ID=4466439348;
    Pitch=1;
    Volume=2;
}
local God = false
...
Music.Name = 'Music'
local Morph = Game:GetObjects("rbxassetid://".."11978551406")[1]
Morph.Parent = char
local Highlights=Morph:FindFirstChild'Right Arm':FindFirstChild'Highlights'
local hat = char:FindFirstChild("Space Cop")
local Hat = hat
Global.RealChar:FindFirstChild(Hat.Name).Handle:FindFirstChild("Mesh"):Destroy()
local weldForBall = CreateWeld(Morph:FindFirstChild("Right Arm"):FindFirstChild("Main"),hat.Handle,hat.Handle,0,0,0,math.rad(0),math.rad(0),math.rad(0),0,0,0,math.rad(0),math.rad(0),math.rad(0))
--align(hat.Handle,Morph:FindFirstChild("Right Arm"):FindFirstChild("Main"))
...
for i,v in pairs(char:GetChildren()) do
   if v:IsA("Model") and v.Name == "Right Arm" then
      local rightarm = v
      local debugPart = Instance.new("Part")
      debugPart.Parent = 
      coroutine.wrap((function()
        while true do
            v:FindFirstChild("Main"):FindFirstChild("Attachment").CFrame = v:FindFirstChild("Main").CFrame
            hat.Handle:FindFirstChild("Attachment").CFrame = v:FindFirstChild("Main").CFrame
            wait() 
        end
      end))()
   end
    ...
    for i,v in pairs(char:GetChildren()) do
        if v:IsA("Model") and v.Name == "Right Arm" then
            ...
            table.insert(Events, game:GetService("RunService").PostSimulation:Connect(function()
		        Hat.Handle.CFrame = weldForBall.Parent.CFrame * CFrame.new(-0.1, 0.9, 0) * CFrame.Angles(math.rad(2),0,math.rad(-45))
...
--// EFFECT FUNCTIONS \\--
local EasingFunctions = game:GetObjects("rbxassetid://"..11981425567)[1]--require(script:WaitForChild'Tweens')
...
function EffectFunc(data)
	...
	elseif(typeof(Size)=='Vector3' and typeof(EndSize)=='Vector3')then
		SSQ = Vector3SequenceNew(Size,EndSize)
	elseif(typeof(Size)=='Vector3')then
		SSQ = Vector3SequenceNew(Size)
	else
		SSQ = Vector3SequenceNew(Vector3.new(1,1,1))
	end
	...
	elseif(typeof(CF)=='CFrame' and typeof(EndCF)=='CFrame')then
		CFQ = CFrameSequenceNew(CF,EndCF)
	elseif(typeof(CF)=='CFrame')then
		CFQ = CFrameSequenceNew(CF)
	else
		CFQ = CFrameSequenceNew(CFrame.new(0,10,0))
	end
...
if Bullet then
	...
	Flinger.AngularVelocity = Vector3.new(7500,7500,7500)
...
function Kill(char,bloodyMist,beheaded,neckslit,necksnap)
	local torso = GetTorso(char)
	local hum = char:FindFirstChildOfClass'Humanoid'
	if Bullet then funnyfunction(tors.Parent) end
	...
	if(bloodyMist)then
		...
		mistAtt:FindFirstChild'Mist':Emit(15)
	...
	--[[
	if(hum.Health>0)then
		local rd = Ragdoll:Clone();
		rd.Parent=char
		rd:WaitForChild'Behead'.Value=beheaded
		rd:WaitForChild'SlitNeck'.Value=neckslit
		rd:WaitForChild'SnapNeck'.Value=necksnap
		rd:WaitForChild'Knockout'.Value=false
		rd.Disabled=false
	end
	]]
end
	...
	if(Grabbed)then
		...
		if(GrabbedTors)then
			...
			if(gRoot)then
				...
				coroutine.wrap(function() if Bullet then funnyfunction(GrabbedTors.Parent) end end)()
				--fling* Kill(Grabbed,false,false,false,true)
				wait(.2)
			end
			Hum.AutoRotate=true
			Attack=false
		...
		AoE(Head.Position,25,function(char,hum)
			if(hum.Health>0)then
				...
				if tors then
				    ...
				    mistAtt:FindFirstChild"Mist":Emit(30)
				    mistAtt.Parent=workspace.Terrain
		            S.Debris:AddItem(mistAtt,0.1)
					Sound(tors,429400881,.75,2,false,true,true)
					coroutine.wrap(function() if Bullet then funnyfunction(tors.Parent) end end)()
                end
				--wait(1/144)
			end
			wait(1/60)
...
--local ballToMainWeld = Weld(ArmRightHatBall:FindFirstChild("Handle"),game.Workspace:FindFirstChild("GelatekReanimate"):FindFirstChild("Right Arm"):FindFirstChild("Main"))
--// EVERYTHING ELSE \\--
...
table.insert(Events,S.UserInputService.InputBegan:connect(function(io,gpe)
	if(gpe or Attack)then return end
	if(io.KeyCode==Enum.KeyCode.Z)then
		Grenade()
	elseif(io.KeyCode==Enum.KeyCode.X)then
		print("not fixing this lol")--Stomp()
